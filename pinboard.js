...
Pinboard.prototype.request = function (method, api_method, params, body, statechange, async) {
	...
	$.ajax({
  	...
  	.done(function(data)
  	{
		...
		{
			item = data[i];
			split_tags = item["tags"].split(' ');
			for (j in split_tags)
			{
				tags[split_tags[j]] = true;
			}
		}
		var tag_string = "";
		$.each(tags, function(key, value)
		{
			tag_string = tag_string + key + ", ";
		});
		msg("All tags: " + tag_string);
	})
	.fail(function(jqXHR, textStatus, errorThrown, asdf)
	{
	 	dbg("Error with " + jqXHR.status);
	});
}
